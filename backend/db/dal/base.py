...
class AsyncPostgreSQLDAL(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    ENSURE_JSON_SAFETY_FIELDS: set[str] = set()

    IMMUTABLE_FIELDS: set[str] = {"id", "created_at"}
    AUTO_UPDATE_FIELDS: dict[str, Callable[[], Any]] = {"updated_at": lambda: utcnow()}

    @classmethod
    def _sanitize_json_fields_on_model(
        cls,
        obj: CreateSchemaType | UpdateSchemaType,
    ) -> None:
        """Mutates obj in-place for any fields listed in SANITIZE_FIELDS, only if the value is a dict."""
        if not cls.ENSURE_JSON_SAFETY_FIELDS:
            return
        for fld in cls.ENSURE_JSON_SAFETY_FIELDS:
            try:
                val = getattr(obj, fld, None)
            except Exception:
                continue
            if isinstance(val, dict):
                sanitized = json_sanitize(val)
                setattr(obj, fld, json_ensure_or_fallback(sanitized))
    ...
    async def create(
        ...
        cls._sanitize_json_fields_on_model(obj_in)
    ...
    async def update_many_by_ids(
        ...
        try:
            ...
            for id_, update_obj in updates.items():
                cls._sanitize_json_fields_on_model(update_obj)
