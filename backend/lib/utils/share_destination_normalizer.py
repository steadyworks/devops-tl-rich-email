...
def normalize_phone_e164_throws(
    raw: str, default_region: str | None = DEFAULT_REGION
) -> str:
    raw = (raw or "").strip()
    if not raw:
...
def normalize_email_throws(raw: str) -> str:
    return raw.lower()
...
def normalize_destination(channel_type: ShareChannelType, value: str) -> str:
    try:
        ...
        if channel_type == ShareChannelType.EMAIL:
            return normalize_email_throws(v)
        if channel_type == ShareChannelType.SMS:
            return normalize_phone_e164_throws(v, DEFAULT_REGION)
